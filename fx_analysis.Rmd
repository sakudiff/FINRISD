...
Data cut-off: **27 February 2026**, per case guidelines. Weekends are removed. The CSV has already been patched for the 2021-05-20 Open typo (478.855 → 47.855).
...
df_raw <- read_csv("data/unified_usdphp_bap.csv") %>%
  ...
  # Remove weekends imputed by the consolidation script (1=Sun, 7=Sat in lubridate)
...
ggplot(df_returns, aes(x = Date)) +
  geom_line(aes(y = intraday_pct,   color = "Intraday Range (H-L)"), linewidth = 0.45, alpha = 0.8) +
  geom_line(aes(y = abs(dod_pct),   color = "Abs. Close-to-Close"),  linewidth = 0.45, alpha = 0.8) +
  scale_color_manual(values = c("Intraday Range (H-L)" = econ_blue,
                                "Abs. Close-to-Close"  = econ_red)) +
  labs(title = "Mind the gap",
       subtitle = "Intraday H-L range (%) vs absolute close-to-close change (%), full history 2019–2026.",
       x = NULL, y = "Magnitude (%)",
       caption = paste0("Full history. Intraday VaR 95% = ",
                        ...
                        "%. Gap shows intra-day volatility that 'recovers' by close.")) +
  theme_economist_quant()
```
...
ggplot(item1_var, aes(x = Window, y = VaR_Pct,
  ...
  labs(title = "Stability in numbers",
       ...
       caption = "Windows: Full = 1,868 obs; w60 = 60 obs; w40 = 40 obs; w20 = 20 obs.") +
