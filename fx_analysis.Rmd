...
Solution 2 is **Winsorization**. Extreme returns are capped at the 0.1th and 99.9th percentile boundaries rather than removed. The full dataset is preserved. Outlier influence is bounded. **VaR after winsorization falls to `r round(var_winsor_pct, 4)`%, a reduction of `r round(raw_var_pct - var_winsor_pct, 4)` percentage points, or `r round(raw_var_cents - var_winsor_cents, 2)` cents.** Winsorization is a robustness check. It quantifies the degree to which outliers inflate the standard VaR. It does not resolve the sub-additivity failure. CVaR remains the preferred primary measure.
...
**The VaR figures for the 20-day, 40-day, and 60-day windows are both statistically and economically distinct from the full 2019–2026 history.** Both dimensions of significance are present. Neither can be dismissed.

The Kolmogorov-Smirnov test evaluates distributional equality between each window and the full history. The KS p-values for w60, w40, and w20 are `r paste(round(item5_tests$KS_p, 4), collapse = ", ")` respectively. The Levene test evaluates variance equality. Levene p-values are `r paste(round(item5_tests$Levene_p, 4), collapse = ", ")` respectively. **Low p-values confirm that the short windows are not drawn from the same statistical population as the full history.** The distributions are detectably different.
...
- **Outlier solutions (§7):** Winsorization (reduces VaR by `r round(raw_var_pct - var_winsor_pct, 4)` pp) bounds extreme influence without deleting data; switching to CVaR renders the specific outliers structurally irrelevant by averaging across the entire tail.
- **Window adequacy (§8):** Short-window VaR (20/40/60 days) is statistically and economically distinct from the full history — driven by recency bias and regime shifts. Standalone use is inadequate; full-history VaR is the appropriate risk baseline.
